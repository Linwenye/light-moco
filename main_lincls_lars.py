...
parser.add_argument('-e', '--evaluate', dest='evaluate', action='store_true',
                    help='evaluate my_modules on validation set')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # create my_modules
    print("=> creating my_modules '{}'".format(args.arch))
    ...
    if args.lars:
        ...
        from moco.utils import LARS
        optimizer = LARS(parameters, init_lr, weight_decay=args.weight_decay, momentum=args.momentum)
    else:
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if args.gpu is None:
                ...
                # Map my_modules to be loaded to specified single gpu.
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    it is not legitimate to change any part of the pre-trained my_modules.
    BatchNorm in train mode may revise running mean/std (even if it receives
    no gradient), which are part of the my_modules parameters too.
    """
    model.eval()
...
def sanity_check(state_dict, pretrained_weights, args):
    ...
    for k in list(state_dict.keys()):
        ...
        # name in pretrained my_modules
        if args.moco or args.msf:
